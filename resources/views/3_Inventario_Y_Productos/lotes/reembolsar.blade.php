<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Información del lote sujeto al proceso de reembolso') }}
        </h2>
    </x-slot>

    <title>Reembolso</title>

    <form action="{{ route('lotes.reembolsaractualizar', $lote->id) }}" method="POST">
        @csrf
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg m-5">
                        ...
                        <div class="grid lg:grid-cols-4 grid-cols-1 gap-4 p-2">
                            ...
                            <div class="col-span-1">
                                ...
                                <input id="cantidad" name="productos[{{ $index }}][cantidad]" type="number" min="1"
                                    class="px-3 py-2 w-full rounded-xl bg-blue-100" value="{{ $loteprod->cantidadComprada }}" placeholder="Cantidad" disabled>
                            </div>
                            <div class="col-span-1">
                            ...
                            <div class="col-span-1">
                                ...
                                <input id="precioCompra" name="productos[{{ $index }}][precioCompra]" type="number" min="1"
                                    class="px-3 py-2 w-full rounded-xl bg-blue-100" value="{{ $loteprod->precioCompra }}" placeholder="Precio de Compra" disabled>
                            </div>
                        </div>
                    @endforeach
                </div>
            ...
            <div class="p-5">
                <button type="submit" class="bg-blue-600 text-white font-bold px-6 py-3 rounded-lg">
                    <i class="fa-solid fa-screwdriver-wrench">  Solicitar proceso de reembolso</i>
                </button>
            </div>
        </div>
    </form>
