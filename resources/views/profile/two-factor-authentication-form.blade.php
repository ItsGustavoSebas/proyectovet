<x-action-section>
    ...
    <x-slot name="description">
        {{ __('Añade una seguridad adicional a tu cuenta usando la autentificación de dos factores.') }}
    </x-slot>
        ...
        <h3 class="text-lg font-medium text-gray-900">
            @if ($this->enabled)
                @if ($showingConfirmation)
                    {{ __('Termino la activación de la autentificación de dos factores.') }}
                @else
                    {{ __('Has activado la autentificación de dos factores.') }}
                @endif
            @else
                {{ __('No has activado la autentificación de dos factores.') }}
            @endif
        </h3>
